...
// Save database with atomic file write
...
function calculateEloGain(winnerElo, loserElo) {
  ...
  return Math.max(1, change); // Minimum 1 Elo point
}
...
const server = http.createServer(async (req, res) => {
  ...
  // 1. GET /api/data -> Fetch shared club data
  ...
  // 2. POST /api/auth/signup -> Register brand new player
  ...
  // 3. POST /api/auth/login -> Sign in to existing account
  ...
  // 4. POST /api/matches -> Report new match
  ...
  // 5. POST /api/matches/:id/confirm -> Confirm match result
  ...
  // 6. POST /api/matches/:id/reject -> Reject / cancel match
  ...
  // 7. POST /api/reset -> Reset database to 0 accounts, 0 matches, 0 players
  if (req.method === "POST" && pathname === "/api/reset") {
    db = JSON.parse(JSON.stringify(INITIAL_DB));
    saveDb(db);
    console.log("[ADMIN] Database reset to clean state (0 players, 0 matches).");
...
server.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(`👥 Registered Players: ${db.players.length}`);
  console.log(`🏓 Recorded Matches: ${db.matches.length}`);
  console.log(`===================================================`);
});
